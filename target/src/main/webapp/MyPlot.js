...
class MyNewScopeCtr {
    ...
    initOpts(md) {
        ...
        opts.typeCnt = 0;
        opts.dispValue = 15;
        // 啟用新一代專業控制器（若未載入則自動回退到舊邏輯）。
        opts.useProfessionalController = true;
        opts.scopePreset = "default";
    ...
    chkWatch() {
        ...
        st.signalButtonColors = [];
        for (var si = 0; si < op.chNames.length; si++) {
            st.signalButtonColors.push("#ccf");
        }
        if (op.signalMode >= 0 && op.signalMode < st.signalButtonColors.length) {
        ...
        //========================================================
        // ProfessionalScopeController 整合（舊邏輯保留為 fallback）
        if (op.useProfessionalController && typeof ProfessionalScopeController === "function") {
            if (!st.scopeCtl) {
                st.scopeCtl = new ProfessionalScopeController({
                    onError: function (msg) {
                        console.log("[ProfessionalScopeController] " + msg);
                    }
                });
                st.scopeCtl.loadPreset(op.scopePreset || "default");
                st.lastScopePreset = op.scopePreset || "default";
            }
            var ctl = st.scopeCtl;
            if (st.lastScopePreset !== (op.scopePreset || "default")) {
                ctl.loadPreset(op.scopePreset || "default");
                st.lastScopePreset = op.scopePreset || "default";
            }
            ctl.state.run = !!scope.opts.run_f;
            ctl.state.display.cursor = !!scope.opts.cursor_f;
            ctl.state.display.grid = !!scope.opts.grid_f;
            ctl.state.display.persist = !!scope.opts.net_f;
            ctl.state.selectedChannel = op.chSelectInx;

            var syncLen = Math.min(scope.opts.lines.length, ctl.state.channels.length);
            for (var ci = 0; ci < syncLen; ci++) {
                var lineObj = scope.opts.lines[ci];
                var chObj = ctl.state.channels[ci];
                chObj.enabled = !!lineObj.offOn_f;
                if (lineObj.yScale !== undefined) {
                    chObj.scale = lineObj.yScale;
                }
                if (lineObj.offset !== undefined) {
                    chObj.offset = lineObj.offset;
                }
                if (lineObj.name) {
                    chObj.name = lineObj.name;
                }
            }

            var bind = ctl.toUiBindings();
            watchDatas[0] = bind.runColor;
            watchDatas[1] = ctl.state.run ? '<i class="gf">&#xe034</i>' : '<i class="gf">&#xe037</i>';
            watchDatas[2] = bind.cursorColor;
            watchDatas[3] = bind.gridColor;
            watchDatas[4] = ctl.state.display.persist ? "#ccf" : "#ccc";
            st.profScopeState = ctl.exportConfig();
        }
    }
    afterCreate() {
        //this.setChAlign();
    ...
    build() {
        ...
        opts.actionFunc = function (iobj) {
            console.log(iobj);
            var mapPresetBySignal = function (signalMode) {
                // 將舊 signalMode 映射到新控制器 preset。
                if (signalMode === 2) {
                    return "pulse";
                }
                if (signalMode === 3) {
            ...
            if (inx === 0) {
                op.signalMode = 0;
                op.signalModeInx = 0;
                op.scopePreset = mapPresetBySignal(op.signalMode);
                var scope = md.fatherMd;
...
/**
 * 四通道示波器：管理資料緩衝、時間視窗、觸發對齊及 Canvas 互動。
 *
 * 框架透過 this.md 注入宿主；md.opts（op）保存設定與各通道資料，
 * md.stas（st）保存像素尺寸、Canvas、拖曳狀態與重繪旗標。
 * 建立流程：initOpts → build（宣告子元件）→ afterCreate（建立 Canvas 與事件）。
 * 更新流程：chkWatch → frameTimer → emuTestWave → getBufs → drawBufs / drawDutyBar。
 *
 * 時間統一以 ns 表示：最新記錄右端是 0，歷史資料位於負時間；
 * zoomTimeEnd 是視窗右端，zoomTimeLen 是整個視窗涵蓋時間。
 * typeCnt=0 為等間隔振幅樣本，取樣間隔為 1e9 / sampleRate ns。
 * typeCnt=1 為脈波長度編碼：(durationNs << 1) | level，
 * 以 >> 1 解碼長度、& 1 取電位；輸入須符合 JavaScript 有號 32 位元運算範圍。
 *
 * ctx 畫網格／標籤、ctx1 畫波形、ctx2 畫游標／記錄範圍導覽列。
 * drawAxe_f、drawBuf_f 是延後到 frameTimer 處理的要求，不是立即繪製。
 * 本類別同時保留新舊資料介面及停用區塊，下方註解描述現況，不修改其行為。
 */
class MyNewScope {
    /** 宿主稍後注入 md；此處不建立緩衝區、DOM 或計時器。 */
    constructor() {



    }
    // 舊式文字時間刻度，對應 transXScale 的輸入格式；現行縮放改用下方數值表。
    static xScaleTblxxx = [
    ...
    // 每一大格的時間候選值（ns）；timeZoomPrg 固定乘以 10 換算全視窗範圍。
    static xScaleTbl = [
        5, 6, 7.5, 8, 10,
    ...
    // 每大格振幅的數值候選表；物理單位由通道的 yScaleUnit 指定。
    static yScaleTbl = [
    ...
    // 舊式控制器的單位字串表；不等同於 getBufs 實際使用的數值 yScale。
    ...
    /**
     * 將「整數 + 空格 + nS/uS/mS/S」換算成 ns，解析失敗回傳 500。
     * @param {string} inStr 舊版文字刻度；直接呼叫 split，不接受數值刻度。
     * @returns {number} 換算時間或預設值 500 ns。
     */
    ...
    /**
     * 將 ns 格式化為時間文字：us 以上保留三位小數，ns 取整數。
     * 單位判斷直接比較 iv 而非其絕對值，主要用於非負脈波長度。
     * @param {number} iv 時間（ns）。
     * @returns {string} 帶 nS/uS/mS/S 單位的字串。
     */
    static transTime(iv) {
        var vstr = "";
        if (iv >= 1000000000) {
    ...
    /**
     * 已停用的舊刻度轉換器，目前立即 return，不會改動 op。
     * 後方保留 yScaleTbl/yScaleSet 換算邏輯；現行繪圖直接讀取數值 yScale。
     */
    ...
    /**
     * 重設操作狀態及「名稱~0/1」標籤，並非單純刷新顯示。
     * 索引 0..5：CLEAR/RUN/TRIG/CURSOR/GRID/NET；6..8：測試；9..12：SIG。
     * 初始化與 CLEAR 都會呼叫，因此也會停止 RUN、關閉觸發與游標。
     */
    initRightTags(opts) {
        opts.clearOn_f = 0;
        opts.run_f = 0;
    ...
    /**
     * 建立並回傳預設選項；md 參數目前未使用，不在此建立 DOM。
     * 位置與軸長採容器千分比，afterCreate 才換算像素；
     * 通道 offset 則是繪圖高度百分比，不可混用。
     * initLines 預設配置四個各 2,000,000 筆的一般 Array。
     */
    initOpts(md) {
        var self = this;
        var opts = {};
        ...
        // 水平觸發點以千分比保存，500 是畫面中央，不是 500 ns。
        ...
        // 預設顯示最新資料前 100 us，即相對時間 [-100000, 0] ns。
        opts.zoomTimeEnd = 0;
    ...
    /**
     * 重新建立四個通道，清空記錄及通道設定；切換信號也會走此路徑。
     * buffer 是環形陣列，endInx 是 addBuf 下一筆寫入位置，
     * recordLen 是有效筆數（上限為容量），serialCnt 是累計寫入筆數。
     * stInx 另供舊式 addLineBuf 使用；兩套索引介面不自動同步。
     * yScale 為每格振幅，offset 是基線位移百分比，trigOffset 與資料同單位。
     */
    initLines(opts) {
        opts.lines = [];
        //===============
    ...
    /** 調整細網格亮度索引並限制在 0..9，同步 NET 標籤及座標重繪要求。 */
    netFadeAdd(addV) {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /** 未傳 value 時反轉觸發開關，有傳則指定；觸發線顯示 30 個更新週期。 */
    trigOnOff(value) {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /**
     * 切換／指定主網格；現有程式寫 rightTags[3]（CURSOR），
     * 與 GRID 原定索引 4 不一致；此處僅註記，未修正。
     */
    gridOnOff(value) {
        var md = this.md;
        var op = md.opts;
    ...
    /**
     * 切換／指定細網格；現有程式寫標籤索引 3 而非 NET 的 5，且省略亮度。
     * 細網格實際是否繪出仍由 net_f 決定。
     */
    netOnOff(value) {
        var md = this.md;
        var op = md.opts;
    ...
    /**
     * 切換／指定邊緣：0 上升、1 下降，由 getBufs 判斷門檻穿越。
     * 現有程式另將 NET 文字寫入 rightTags[3]，並非邊緣狀態的顯示。
     */
    trigUpDown(value) {
        var md = this.md;
        var op = md.opts;
    ...
    /**
     * 切換／指定 RUN，停止後保留資料，仍能縮放、平移與使用游標。
     * addBuf 與 emuTestWave 會檢查 run_f；舊式 addLineBuf 本身不檢查。
     */
    runOnOff(value) {
        var md = this.md;
        var op = md.opts;
    ...
    /**
     * 切換來源、重建所有通道，並同步右側標籤與重繪要求。
     * mode 0 停止；1 為三種測試；2、3 分別設定 SIG1、SIG2。
     * mode 4、5 僅選取標籤並啟動 RUN，此處無對應通道設定分支。
     * @param {number} signalMode 模式代碼。
     * @param {number} signalModeInx 模式內子索引，測試模式使用 0..2。
     */
    ...
    /**
     * 以滑鼠／旋鈕指定的水平位置為錨點，切換到相鄰的十格時間範圍。
     * 先求錨點時間，再調整視窗右端，使同一時間維持在原水平比例。
     * @param {number} zoomPosRate 0..1 的水平比例，超出即忽略。
     * @param {number} value 正值擴大時間範圍，非正值縮小；僅看方向。
     */
    ...
    /** 開始圖內時間平移，保存起點比例並清除位移與慣性；尚不提交視窗右端。 */
    ...
    /** 將拖曳比例換算成時間並提交至 zoomTimeEnd，結束圖內平移。 */
    timePosEndPrg() {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /**
     * 更新拖曳預覽 zoomPosRateDt，不直接修改 zoomTimeEnd；由 getBufs 套用。
     * 邊界允許在記錄前後各多出視窗時間的 60%，避免完全失去記錄位置。
     * maxRecordLenTime 由 drawDutyBar 更新，作為記錄左界的依據。
     */
    ...
    /**
     * 設定目前觸發通道的門檻，yRate 以大格數表示，乘 yScale 得到資料值。
     * delta_f 為真則累加，否則絕對設定；同時更新觸發線倒數與重繪旗標。
     */
    ...
    /** 將指定通道的每格振幅改為相鄰候選值：正值擴大範圍，非正值縮小。 */
    setYScale(addV, inx) {
        var self = this;
        var md = this.md;
        var op = md.opts;
    ...
    /**
     * 絕對模式把由上到下的 0..1 轉成基線位移 +50..-50 百分比。
     * 增量模式直接累加 offset 百分比；此處不限制增量結果範圍。
     */
    ...
    /**
     * 容器完成後換算像素尺寸、疊放三張 Canvas 並綁定滑鼠操作。
     * 波形層與互動層同為 z-index 1；較晚加入的互動層在上方接收事件。
     * noRectA 是命中區陣列，noRectAOnFlag/noRectADragFlag 以位元遮罩追蹤。
     * 最後發送 afterCreate 回呼；實際繪製仍由 frameTimer 處理。
     */
    afterCreate() {
        var self = this;
        var md = this.md;
        ...
        // 先處理按鈕，再選擇圖內平移、通道／觸發標記或導覽列拖曳。
        // 每 100 ms 輪詢 gr.mouseDown_f 是清理後備，不像 canvasUpFunc 會提交平移。
        ...
        // 在 Canvas 內放開時提交導覽列或圖內平移量，再清除拖曳暫存。
        ...
        // 先以像素矩形建立 hover 遮罩，再依拖曳種類更新設定並提前返回。
        // 標記拖曳優先於導覽列，導覽列優先於圖內時間平移。
        ...
        // 依位置分流：TRIG 選通道、觸發標記換邊緣、NET 調亮度、通道調 Y 刻度；其餘縮放時間。
        ...
        selem.addEventListener("mousemove", canvasMoveFunc);
        plotElem.appendChild(selem);
        // 最上層接收所有滑鼠事件；未畫游標時，這張 Canvas 仍覆蓋整個容器。
        st.canvasLy2 = selem;
        //=========================================
    ...
    /** 設定四通道正弦測試：200 MHz、20 mV/格、100 us 視窗；此處不寫入樣本。 */
    setTest1Signal() {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /** 設定四通道類比隨機值測試：20 MHz、10 mV/格、100 us 視窗。 */
    setTest2Signal() {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /** 設定四通道脈波長度編碼測試：typeCnt=1，高電位顯示值 levelSize=10。 */
    setTest3Signal() {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /** 設定 SIG2 四路功率讀值：60 筆/秒、DB 單位、1 秒視窗，來源 gr.plotValue。 */
    setM3I0Signal() {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /**
     * 設定 SIG1：只開第一通道，使用脈波長度編碼、levelSize=15、1 ms 視窗。
     * emuTestWave 的對應產生器已提前返回；此方法只設定，不自行供應資料。
     */
    setM2I0Signal() {
        var md = this.md;
        var op = md.opts;
        ...
        for (var i = 0; i < 4; i++) {
            ...
            lineObj.valueViewFixed = 0;
            // 保留原式：逗號運算使實際 offset 為 -7，不是小數 -7.5。
            lineObj.offset = -7, 5;
    ...
    /** 框架週期更新入口：執行 frameTimer，並將 ani.dispFs 寫入全域頁尾狀態。 */
    chkWatch() {
        var md = this.md;
        var op = md.opts;
        var st = md.stas;
    ...
    /**
     * 每輪先更新資料、觸發線倒數與慣性平移，再按旗標重繪。
     * RUN 時先對觸發通道預掃描，求出四通道共用的 zoomTimeTrig，
     * 再計算四通道顯示資料，最後繪製波形及記錄範圍導覽列。
     * 停止時不重新搜尋觸發而保留對齊；關閉觸發時 getBufs 將位移歸零。
     * 倒數與慣性衰減依呼叫次數計算，並非實際經過毫秒數。
     */
    frameTimer() {
        var self = this;
        var md = this.md;
        ...
        // 放開圖內拖曳後延續最後水平速度，每三輪往 0 衰減一步，並限制在記錄附近。
        if (!st.dragPosOn_f) {
        ...
        if (st.drawBuf_f) {
            ...
            if (op.run_f) {
                ...
                // 預掃描僅決定共用觸發位移，下方四次 getBufs 才建立正式顯示資料。
                if (op.trig_f)
                    self.getBufs(opts, 1);
            }
    ...
    /**
     * 寫入 endInx 式環形緩衝，滿載後覆寫最舊資料，並要求重畫波形。
     * 只有 RUN 且通道開啟時接收；呼叫端須提供有效索引與正確資料格式。
     * @param {number} lineInx 通道索引 0..3。
     * @param {number[]} values 振幅樣本，或 typeCnt=1 的時間／電位編碼。
     */
    ...
    /**
     * 每輪依模式供應資料，並將模式同步到全域 gr。
     * 測試 0 為正弦、1 為隨機振幅、2 為交替電位的隨機脈波長度。
     * mode 2 的舊模擬路徑已停用；mode 3 讀取 gr.plotValue。
     * mode 3 以每秒 62 次更新估算筆數，不依實際時鐘校正。
     */
    emuTestWave() {
        var self = this;
        var md = this.md;
        ...
        // 此分支立即 return，後方 gr.pulseFormAA/gr.pulseLevelAA 舊演算法目前不執行。
    ...
    /**
     * 舊式起點寫入法：使用 stInx + recordLen，達 sampleAmt + 1 筆後推進起點。
     * 不檢查 RUN 或通道開關，也不更新 endInx/serialCnt；mode 3 仍呼叫它。
     * getBufs 使用 endInx - recordLen 求起點，故不能視此方法與 addBuf 等價。
     * @param {number[]} buf 待寫入資料。
     * @param {number} inx 通道索引；參數順序與 addBuf 相反。
     */
    addLineBuf(buf, inx) {
        var self = this;
        var md = this.md;
    ...
    /** 清除三張 Canvas 的像素，不修改 buffer、索引或重繪旗標。 */
    clearAll() {
        var st = this.md.stas;
        var ctx2 = st.ctx2;
        var ctx1 = st.ctx1;
    ...
    /**
     * 將環形記錄轉成每水平像素一筆的顯示資料，或執行觸發預掃描。
     * @param {Object} opts 單一通道物件，不是整體 md.opts。
     * @param {number} [trigPrg_f] 真值表示搜尋觸發，最多往左掃兩個視窗寬。
     *
     * infDataA 由右到左排列，包含 value、x/y、nowTime、峰值像素 max/min；
     * 脈波另存 tValue/recInx，讓游標合併同電位的不同記錄。
     * 最新記錄之後以 null 佔位，超過最舊記錄即停止，不補造樣本。
     * 類比資料以線性內插取得值，縮小時另收集中間極值；脈波依時間段解碼。
     * 觸發成功只更新 st.zoomTimeTrig 後返回，不提交該次 infDataA；
     * frameTimer 隨後會再用正式顯示模式計算所有通道。
     */
    getBufs(opts, trigPrg_f) {
        ...
        //ctx.beginPath();
        var winYcen = st.containerHeight - st.xyOffy - st.yAxeLen / 2;
        var yGridLen = st.yAxeLen / op.yAxeGridAmt;
        var winYOffset = st.yAxeLen * opts.offset / 100;
        //============================================
        ...
        // endInx 指向下一筆寫入槽；往回 recordLen 筆才是最舊有效資料。
        if (!op.run_f) {
            if (!st.recEndInxBuf)
                st.recEndInxBuf = opts.endInx;
            if (!st.recordLenBuf)
                st.recordLenBuf = opts.recordLen;
            recEndInx = st.recEndInxBuf;
            recLen = st.recordLenBuf;
            var recInxB = recEndInx;
            if (--recInxB < 0)
                recInxB = recBufSize - 1;
            if (!st.recEndValueBuf)
                st.recEndValueBuf = opts.buffer[recInxB];
            opts.buffer[recInxB] = st.recEndValueBuf
        }
        else {
            st.recEndInxBuf = opts.endInx;
            st.recordLenBuf = opts.recordLen;
            var recInxB = st.recEndInxBuf;
            if (--recInxB < 0)
                recInxB = recBufSize - 1;
            st.recEndValueBuf = opts.buffer[recInxB]
        ...
        // 脈波編碼不是固定取樣間距，總時間須累加每筆 durationNs，不能使用 sampleRate。
        if (opts.typeCnt === 1) {
            var recTimeLen = 0;
        ...
        if (!op.trig_f)
            st.zoomTimeTrig = 0;
        // 套用導覽列預覽及觸發位移，稍後再加上尚未提交的圖內拖曳預覽。
        if (op.run_f) {
            st.zoomTimeDelta = 0;
            var zoomTimeEnd = 0;
            var zoomTimeStart = zoomTimeEnd - op.zoomTimeLen;
            var zoomTimeMid = (zoomTimeEnd + zoomTimeStart) >> 1;
            ...
            if (trigPrg_f) {
                if (opts.typeCnt === 1) {
                    ...
                    for (var i = 0; i < recLen; i++) {
                        ...
                        if (!op.trigUpDown_f) {
                            ...
                            recTime -= period;
                            if (recTime > zoomTimeMid)
                                continue;
                            st.zoomTimeTrig = (recTime * (-1) - (zoomTimeLen >> 1)) + pointTime * 2;
                            return;
                        }
                        else {
                            if (value & 1) {
                ...
                var zoomTimeEnd = 0 - st.zoomTimeTrig;
                var zoomTimeStart = zoomTimeEnd - op.zoomTimeLen;
                var zoomTimeMid = (zoomTimeEnd + zoomTimeStart) >> 1;
            }
        ...
        if (md.stas.dragPosOn_f) {
            var deltaTime = md.stas.zoomPosRateDt * zoomTimeLen;// 計算拖曳位移對應的時間偏移量
        ...
        for (var ii = 0; ii < maxLen; ii++, nowTime -= pointTime) {
            ...
            if (opts.typeCnt === 0) {
                ...
                // 一像素跨越多筆類比資料時，另收集中間極值，避免內插遺漏窄尖峰。
                if (deltaInxLen > 2) {
                ...
                if (inxBuf >= recBufSize)
                    inxBuf -= recBufSize;
                // 最新樣本不向下一槽內插，避免取用尚未寫入或已過期的內容。
                if (inxBuf === st.recEndInxBuf)
                    recInx1 = recInx0;
            ...
            if (opts.typeCnt === 1) {
                ...
                // 第一個有效像素由最新脈波回找所屬時間段，後續再利用前次位置走訪。
                if (!first_f) {
                    ...
                    for (var i = 0; i < recLen; i++) {
                        ...
                        if ((recTime - tLen) < nowTime)// 如果回溯到的時間段已經早於當前時間，則停止回溯
                            break;
                        recTime -= tLen;
                        if (--recInx < 0)
                            recInx = recBufSize - 1;
            ...
            // Canvas Y = 中心 - 振幅格數 × 每格像素 - 基線位移，最後裁到繪圖上下界。
            if (max !== null) {
                var ylen = max * yGridLen / opts.yScale;
                var realY = winYcen - ylen - winYOffset;
                if (realY > maxY)
                    realY = maxY;
                if (realY < minY)
                    realY = minY;
                ...
                if (trigPrg_f) {
                    if (nowX <= trigX) {
                        if (preV !== null && trigV !== null) {
                            var ok_f = 0;
                            // 由右向左掃描，preV 比 vv 晚；preV > 門檻 > vv 是時間正向的上升緣。
                            // 嚴格不等式不包含恰等於門檻的情況，觸發搜尋精度為一個水平像素。
                            if (!op.trigUpDown_f) {
                                if (preV > trigV && trigV > vv)// 當前值 vv 低於門檻且前一值 preV 高於門檻，符合上升緣條件
                                    ok_f = 1;
                            } else {
    ...
    /**
     * 將 getBufs 的像素資料畫到 ctx1，略過 null 並串接主值／極值保留尖峰。
     * 啟用通道須已準備 infDataA；本方法不讀取原始緩衝，也不清除圖層。
     */
    drawBufs(opts) {
        var op = this.md.opts;
        var st = this.md.stas;
    ...
    /**
     * 清除底層與互動層，重建刻度、網格、通道／操作標籤及觸發標記。
     * 按繪製順序產生 noRectA；滑鼠事件依相同索引的位元遮罩命中。
     * 停止時另畫游標；清除互動層也會清掉導覽列，後者由波形更新路徑重畫。
     */
    drawAxe() {
        var op = this.md.opts;
        var st = this.md.stas;
        ...
        //draw 1> 2> 3> 4> 
        // 索引契約：0..N-1 基線、N..2N-1 通道開關、2N 起 13 個操作標籤，再接觸發 Y/X。
        // 觸發標記目前條件式 push，隱藏時不補空槽；選用關閉的觸發通道時，
        // 實際陣列位置可能與事件端使用的固定索引不同。
        st.noRectA = [];
        var nextPos = 0;
        for (var i = 0; i < op.lines.length; i++) {
            var noRect = {};
        ...
        if (op.cursor_f && !op.run_f) {
            ...
            var first_f = 0;
            // infDataA 由右向左排列，滑鼠 X 須反向換算，不能直接當作陣列索引。
            ...
            for (var i = 0; i < op.lines.length; i++) {
                ...
                if (op.lines[i].typeCnt === 1) {
                    // 向陣列兩側累加同電位的不同記錄，用 recInx 避免同段被多像素重複計時。
                    // 僅走訪現有像素資料，未顯示或未取到的記錄不保證涵蓋，並非全緩衝脈寬量測。
    ...
    /**
     * 顯示目前視窗在整段記錄中的位置，不是脈波占空比。
     * 各通道 recTimeLen 的最大值存入 maxRecordLenTime，供平移邊界計算。
     * 色塊至少 20 像素寬，dutyBarRect 保存可拖曳的命中矩形。
     * 使用 zoomTimeEnd - zoomTimeDelta，未另套觸發位移或圖內拖曳預覽。
     */
    drawDutyBar() {
        var op = this.md.opts;
        var st = this.md.stas;
    ...
    /** 預留事件入口，目前只記錄事件；實際控制器分流位於 build 的回呼閉包。 */
    actionFunc(iobj) {
        console.log(iobj);
        if (iobj.act === "mouseClick") {
    ...
    /**
     * 宣告標題、繪圖容器、右側 MyNewScopeCtr 與頁尾的版面及子元件。
     * 將宿主掛到 gr.wavePageObj，並把控制器事件接到本類別的操作方法。
     * 此處只建立框架描述資料，Canvas 等 afterCreate 才附加到容器 DOM。
     */
    build() {
        var self = this;
        var md = self.md;
        ...
        opts.actionFunc = function (iobj) {
            ...
            if (iobj.act === "tunerChange") {
                // 旋鈕相對上次採納角度跨過門檻才調一階；X 平移則連續映射角度。
            ...
            // 舊控制器欄位不等同於現行 zoomTimeLen／數值 yScale，不代表已同步換算。
        ...
        opts.trigInx = 0;
        opts.dispValue = 15;
        // 傳入控制器初始設定，其 actionFunc 閉包操作的仍是本示波器 op/st。
