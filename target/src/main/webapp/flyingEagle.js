class FlyingEagleWeb {
    constructor() {
        ...
        gr.flyingEagleData = {};
        gr.socketRetPrgTbl["ret~tick"] = function (tickData) {
            if(tickData.nowAct!==undefined)
                gr.footBarStatus1=tickData.nowAct;
            ...
            var payload=tickData["payload"];
            var keys = Object.keys(payload);
            for (var i = 0; i < keys.length; i++) {
                var strA = keys[i].split("#");
                if (strA.length === 1) {
                    gr.flyingEagleData[keys[i]] = payload[keys[i]];
                    continue;
                }
                if (strA.length === 2) {
                    var inx0 = KvLib.toInt(strA[1], 0);
                    gr.flyingEagleData[strA[0]][inx0] = payload[keys[i]];
                    continue;
                }
            }
    ...
    chkWatch() {
        ...
        if(gr.flyingEagleData.rxed_f){
            var fd = gr.flyingEagleData;
            st.testInxStr = "TestInx: " + fd.testInx;
            st.modeInxStr = "ModeInx: " + fd.modeInx;
        }
    ...
    setPrg(title, buttonId) {
        ...
        if (buttonId === "testInx") {
            ...
            var opts = {};
            opts.act = "selfDataChange";
            opts.paras = { dataName: "testInx",changeValue: changeValue };
            ws.sendSocket(opts);
            return;
        }
        ...
        if (buttonId === "modeInx") {
            ...
            var opts = {};
            opts.act = "selfDataChange";
            opts.paras = { dataName: "modeInx",changeValue: changeValue };
            ws.sendSocket(opts);
            return;
        }
    ...
    build() {
        ...
        opts.actionFunc = function (iobj) {
            ...
            if (iobj.buttonText == "PlotSecData") {
                ...
                opts.responseType = "errorDialog";
                opts.paras = { testInx: md.opts.testInx, modeInx: md.opts.modeInx };
                ws.sendSocket(opts);
            ...
            if (iobj.buttonText == "DeepLearning") {
                ...
                opts.responseType = "errorDialogOkMessage";
                opts.responseWaitTime = 50;
                opts.paras = { testInx: md.opts.testInx, modeInx: md.opts.modeInx };
                ws.sendSocket(opts);
                return;
            }
        ...
        opts.buttons[0] = "TestInx";
        opts.buttonIds[0] = "testInx";
        opts.buttons[1] = "ModeInx";
        opts.buttonIds[1] = "modeInx";
