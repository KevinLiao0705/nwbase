...
var CodeMirrorLib = {
    ...
    load: function() {
        if (typeof CodeMirror !== "undefined")
            return Promise.resolve(CodeMirror);
        if (this.loadPromise)
            return this.loadPromise;

        var cssPromise = this.loadAsset(
            "codemirror-dynamic-style",
            "link",
            "vendor/codemirror/codemirror.bundle.css"
        );
        var jsPromise = this.loadAsset(
            "codemirror-dynamic-script",
            "script",
            "vendor/codemirror/codemirror.bundle.js"
        );

        this.loadPromise = Promise.all([cssPromise, jsPromise]).then(function() {
            if (typeof CodeMirror === "undefined")
                throw new Error("CodeMirror library did not initialize");
            return CodeMirror;
        }).catch(function(error) {
            CodeMirrorLib.loadPromise = null;
            throw error;
        });
        return this.loadPromise;
    },

    loadAsset: function(id, tagName, url) {
        var existingAsset = document.getElementById(id);
        if (existingAsset)
            return Promise.resolve(existingAsset);

        return new Promise(function(resolve, reject) {
            var asset = document.createElement(tagName);
            asset.id = id;
            asset.onload = function() { resolve(asset); };
     ...
     * @returns {Promise<CodeMirror>} CodeMirror 編輯器實例
     */
